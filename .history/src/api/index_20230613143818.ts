...
// 查
export const set_promotions_type = (params:object) => {
    // 返回的数据格式可以和服务端约定
    return service.get('/mall/sms-flash-promotion-session/one/'+params.id);
}
...
export const del_promotions_type = (params:object) => {
    // 返回的数据格式可以和服务端约定
    return service.get('/mall/sms-flash-promotion-session/del/'+params.id);
}
